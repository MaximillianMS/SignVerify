/*
 * Copyright(C) 2000-2012 Проект ИОК
 *
 ...
 * \date $Date: 2013-03-04 12:24:39 +0400 (Mon, 04 Mar 2013) $
...
#ifdef WITH_STRIBOG
/* Stribog hash sub ids */
#define ALG_SID_STRIBOG256			33
#define ALG_SID_STRIBOG512			34
#endif //WITH_STRIBOG
...
#ifdef WITH_STRIBOG
#define CALG_STRIBOG256 \
    (ALG_CLASS_HASH | ALG_TYPE_ANY | ALG_SID_STRIBOG256)

#define CALG_STRIBOG512 \
    (ALG_CLASS_HASH | ALG_TYPE_ANY | ALG_SID_STRIBOG512)
#endif // WITH_STRIBOG
...
#define CP_DHEL_ALGORITHM	"Diffie-Hellman EL"
...
#define BCRYPT_CP_DHEL_ALGORITHM CAT_L(CP_DHEL_ALGORITHM)
 ...
 * В мультипактном режиме допускается использование предварительной диверсификации ключа, 
 * определяемой флагом CP_CHP_STARTMIX.
 ...
 * (использования флага CP_CHP_MULTIPACKET). Мультипакет передаётся массивом структур CSP_Multipacket.
 * Каждая структура передаёт пакет как линейный буфер либо как IOVEC если в 
 * параметре dwFlags устаны флаги CP_CRYPT_HASH_PACKET, CP_CHP_MULTIPACKET и  
 * CP_CRYPT_DATA_IOVEC в случае представления пакета через IOVEC.
 *  
 */
